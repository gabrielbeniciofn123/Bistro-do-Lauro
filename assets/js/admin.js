(function () {
  ...
  function renderResource(resource, query = "") {
    ...
    const row = {
      areas: (item) => `<tr><td><strong>${PDV.escapeHtml(item.name)}</strong></td><td>${Number(item.table_count)} mesa(s)</td><td>${item.sort_order}</td><td>${statusBadge(item.active)}</td><td>${actionButtons(resource, item.id)}</td></tr>`,
